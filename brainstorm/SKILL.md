...
name: brainstorm
description: >-
  Five-voice adversarial review (Wayfarer, Loremaster, Doomsayer, Alchemist, Marshal) that cross-examines itself, then an Arbiter issues a binding ruling. HARD PREFIX GATE ONLY — use if and only if the user's message begins with the literal prefix "brainstorm:" (the word "brainstorm" immediately followed by a colon, case-insensitive, nothing before it but optional leading whitespace). The plain word "brainstorm" is NOT a trigger on its own, and a colon elsewhere in the message doesn't count — "let's brainstorm", "brainstorm some ideas", "brainstorm names for X", and "so, brainstorm: what should I do" (colon not the first token) are ordinary ideation requests and get an ordinary response. Also not triggered by requests for feedback, critique, review, a second opinion, red-teaming, stress-testing, "what am I missing", "is this a good idea", or "roast this". Only a message starting with "brainstorm:" invokes it — nothing else does, regardless of topic match.
---
